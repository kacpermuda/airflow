...
def get_lineage_from_sql_extras(
    ...
    **multiple** extras only child events are emitted and ``None`` is returned.
    """
    if not sql_extras:
        return None
    ...
    task_lineage_result: OperatorLineage | None = None
    query_count = 0
    ...
    for extra_info in sql_extras:
        ...
        # Only keep the result when this is the sole query; clear on second.
        if query_count == 1:
            task_lineage_result = query_lineage
        else:
            task_lineage_result = None
    ...
    if task_lineage_result is not None:
        log.info("OpenLineage attaching single-query SQL metadata to the task event.")
    return task_lineage_result
