...
class ExtractorManager(LoggingMixin):
            ...
            try:
                ...
                if (not task_metadata.inputs) and (not task_metadata.outputs):
                    ...
                    if hook_lineage is not None:
                        task_metadata.inputs = hook_lineage.inputs
                        task_metadata.outputs = hook_lineage.outputs
                        # We do not want to overwrite facets - only add if not already present.
                        for k, v in hook_lineage.run_facets.items():
                            task_metadata.run_facets.setdefault(k, v)
                        for k, v in hook_lineage.job_facets.items():
                            task_metadata.job_facets.setdefault(k, v)
    ...
    def get_hook_lineage(
        ...
        * **SQL-based** lineage from ``sql_job`` extras reported via
        ...
        if sql_extras:
            from airflow.providers.openlineage.utils.sql_hook_lineage import get_lineage_from_sql_extras
        ...
        result: OperatorLineage = OperatorLineage(inputs=inputs, outputs=outputs)
        if sql_lineage:
