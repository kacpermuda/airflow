...
def check_events_number_func():
    for event_type in ("start", "complete"):
        events = Variable.get(key=f"openlineage_basic_mapped_dag.add_one.event.{event_type}", deserialize_json=True)
        if len(events) != 2:
            raise ValueError(f"Expected exactly 2 {event_type.upper()} events for task `add_one`, got {len(events)}")


DAG_ID = "openlineage_basic_mapped_dag"
...
with DAG(
    ...
    @task(max_active_tis_per_dagrun=1)  # Execute sequentially and do not overwrite each other OL events
    def add_one(x: int):
        return x + 1
