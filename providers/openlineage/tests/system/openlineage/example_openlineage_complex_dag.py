...
from system.openlineage.expected_events import get_expected_event_file_path, AIRFLOW_VERSION
from system.openlineage.operator import OpenLineageTestOperator

def check_events_number_func():
    for event_type in ("start", "complete", "fail"):
        try:
            Variable.get(key=f"openlineage_complex_dag.task_0.event.{event_type}", deserialize_json=True)
        except Exception:
            pass
        else:
            raise ValueError(f"Expected no events for task `task_0`.")
...
class SomeCustomOperator(BashOperator):
    ...
    def __init__(self, **kwargs):
        self.deferrable = True  # Just to test that this attr is included in OL event
        super().__init__(**kwargs)


DAG_ID = "openlineage_complex_dag"
...
with DAG(
    ...
    task_1 = BashOperator(task_id="task_1.id.with.dots", bash_command="exit 0;", owner="owner2", execution_timeout=timedelta(seconds=456))
    task_2 = PythonOperator(task_id="task_2", python_callable=do_nothing, inlets=[Asset(uri="s3://bucket2/dir2/file2.txt")], max_retry_delay=42)
    task_3 = EmptyOperator(task_id="task_3", outlets=[Asset(uri="s3://bucket/dir/file.txt")])
    task_4 = SomeCustomOperator(task_id="task_4", bash_command="exit 0;", owner="owner3", max_active_tis_per_dag=7, max_active_tis_per_dagrun=2)
    ...
    with TaskGroup("section_1", prefix_group_id=True) as tg:
        ...
        with TaskGroup("section_2", parent_group=tg, tooltip="group_tooltip") as tg2:
            if AIRFLOW_VERSION.major == 3:
                ...
                add_args = {"sla": timedelta(seconds=123)}  # SLA is not present in AF3 yet
