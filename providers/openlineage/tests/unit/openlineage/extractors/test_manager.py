...
_SQL_FN_PATH = "airflow.providers.openlineage.utils.sql_hook_lineage.get_lineage_from_sql_extras"
...
def test_get_hook_lineage_with_sql_extras_single_query(hook_lineage_collector):
    hook = MagicMock()
    ...
    sql_facet = MagicMock()
    ext_query_facet = MagicMock()
    sql_lineage = OperatorLineage(
        ...
        run_facets={"externalQuery": ext_query_facet},
    )

    extractor_manager = ExtractorManager()
    with patch(_SQL_FN_PATH, return_value=sql_lineage):
...
@skip_if_force_lowest_dependencies_marker
def test_get_hook_lineage_merges_sql_and_asset_lineage(hook_lineage_collector):
    hook = MagicMock()
    hook_lineage_collector.add_input_asset(None, uri="s3://bucket/input_key")
    hook_lineage_collector.add_extra(
        context=hook,
    ...
    sql_facet = MagicMock()
    sql_lineage = OperatorLineage(
        inputs=[OpenLineageDataset(namespace="db", name="src")],
        outputs=[OpenLineageDataset(namespace="db", name="tbl")],
        job_facets={"sql": sql_facet},
    )
    ...
    assert result == OperatorLineage(
        inputs=[
            ...
            OpenLineageDataset(namespace="db", name="src"),
        ],
        outputs=[OpenLineageDataset(namespace="db", name="tbl")],
        job_facets={"sql": sql_facet},
    )
...
@skip_if_force_lowest_dependencies_marker
def test_get_hook_lineage_sql_extras_multiple_queries_returns_none_for_sql(hook_lineage_collector):
    hook = MagicMock()
    hook_lineage_collector.add_input_asset(None, uri="s3://bucket/input_key")
    hook_lineage_collector.add_extra(
        context=hook,
    ...
    extractor_manager = ExtractorManager()
    with patch(_SQL_FN_PATH, return_value=None):
        result = extractor_manager.get_hook_lineage(
            task_instance=MagicMock(),
            task_instance_state=TaskInstanceState.SUCCESS,
        )
...
def test_get_hook_lineage_passes_failed_state(hook_lineage_collector):
    ...
    extractor_manager = ExtractorManager()
    with patch(_SQL_FN_PATH) as mock_sql_fn:
        mock_sql_fn.return_value = OperatorLineage()
        extractor_manager.get_hook_lineage(
            task_instance=mock_ti,
            task_instance_state=TaskInstanceState.FAILED,
        )

    mock_sql_fn.assert_called_once()
    call_kwargs = mock_sql_fn.call_args.kwargs
    assert len(call_kwargs) == 3
    assert call_kwargs["task_instance"] is mock_ti
    assert call_kwargs["is_successful"] is False
    assert len(call_kwargs["sql_extras"]) == 1
