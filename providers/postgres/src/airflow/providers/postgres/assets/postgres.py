...
def create_asset(
    *, host: str, database: str, schema: str, table: str, port: int = 5432, extra: dict | None = None
) -> Asset:
    return Asset(uri=f"postgres://{host}:{port}/{database}/{schema}/{table}", extra=extra)
...
def convert_asset_to_openlineage(asset: Asset, lineage_context) -> OpenLineageDataset:
    ...
    return OpenLineageDataset(namespace=f"postgres://{parsed.netloc}", name=f"{database}.{schema}.{table}")
