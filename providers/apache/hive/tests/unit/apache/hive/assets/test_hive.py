...
class TestSanitizeUri:
    def test_valid_uri(self):
        result = sanitize_uri(urlsplit("hive://host:10000/default/mytable"))
        assert result.scheme == "hive"

    def test_missing_host(self):
        with pytest.raises(ValueError, match="must contain a host"):
            sanitize_uri(urlsplit("hive:///db/table"))

    def test_missing_path(self):
        with pytest.raises(ValueError, match="must contain a database and table"):
            sanitize_uri(urlsplit("hive://host:10000"))


class TestCreateAsset:
    def test_basic(self):
        asset = create_asset(host="myhost", database="default", table="mytable")
        assert asset.uri == "hive://myhost:10000/default/mytable"

    def test_custom_port(self):
        asset = create_asset(host="myhost", port=10001, database="db", table="t")
        assert asset.uri == "hive://myhost:10001/db/t"
...
class TestConvertAssetToOpenlineage:
    def test_basic(self):
        ...
        assert result.namespace == "hive://myhost:10000"
