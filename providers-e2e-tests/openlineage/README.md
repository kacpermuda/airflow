...
Docker must be running. It is a single breeze command.

```bash
# Default: PROD image (current Airflow + providers from sources)
breeze testing providers-e2e-tests openlineage
...
# Supported released 3.x line: 3.0.6, 3.1.8, 3.2.2
breeze testing providers-e2e-tests openlineage --airflow-version 3.1.8

# Tear the stack down
breeze testing providers-e2e-tests openlineage --down
```

Useful flags:

- `--skip-docker-compose-deletion` — keep the stack up after the run to inspect it (UI at
  <http://localhost:8080>, login `airflow` / `airflow`).
- `--skip-mounting-local-volumes` — run against the image only (what CI does). Omit it during dev to
  mount and hot-reload local `airflow-core` / `task-sdk` / `openlineage` sources.
- Anything after the provider is passed through to pytest (e.g. `... openlineage -- -s`).

The first run builds the PROD image (~20–40 min); subsequent runs are ~6–7 min. When running
non-interactively without a pre-built image, prefix `ANSWER=yes` so the image-build prompt
auto-confirms.
...
- **Per-DAG logs** are written to `logs/dag_id=<dag>/.../task_id=check_events/attempt=1.log`. The
  `check_events` task logs the exact mismatch (``Path `job > facets > ...`: expected X but got Y``),
  which is the fastest way to see *why* a DAG failed.
- With `--skip-docker-compose-deletion`, query the live API: get a token from
  `curl -s http://localhost:8080/auth/token` and check `GET /api/v2/importErrors` for DAG parse
  errors, or browse the runs in the UI.
...
Runs automatically: the PROD (3.4) suite on every relevant PR (core / task-sdk / openlineage /
common-providers changes), and the `3.0.6` / `3.1.8` / `3.2.2` matrix on canary/main runs (each on
its own runner, in parallel). See `.github/workflows/openlineage-e2e-tests.yml`.
