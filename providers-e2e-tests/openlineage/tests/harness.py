...
class OpenLineageE2ERunner:
    ...
    def _trigger_and_wait(self, dag_ids: list[str], run_id: str) -> dict[str, str]:
        ...
        for dag_id in ordered:
            if dag_id not in statuses:
                statuses[dag_id] = self.wait_for_dag_run_to_complete(dag_id, run_id)
        return statuses
    ...
    def run(self, expected_dag_ids: set[str]) -> dict[str, str]:
        ...
        if failed:
            ...
            retry_run_id = f"{self.run_id}_retry1"
            retry_statuses = self._trigger_and_wait(failed, retry_run_id)
            self.clear_airflow_variables()
